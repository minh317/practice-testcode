...
//                .type(HANDMADE) // 타입을 Null로 설정
    ...
    void createProductWithoutSellingStatus() throws Exception {
        ...
        ProductCreateRequest request = ProductCreateRequest.builder()
                .type(HANDMADE)
//                .sellingStatus(SELLING) // 판매 상태를 Null로 설정
                .name("아메리카노")
                .price(4000)
                .build();
    ...
    @DisplayName("신규 상품을 등록할 때 상품 명은 필수 값이다.")
    @Test
...
//                .name("아메리카노") // 상품 명을 Null로 설정
        ...
        _mockMvc.perform(  // perform : API를 수행한다.
                ...
                .andExpect(jsonPath("$.code").value("400"))
                .andExpect(jsonPath("$.status").value("BAD_REQUEST"))
                .andExpect(jsonPath("$.message").value("상품 명은 필수 입니다."))
                .andExpect(jsonPath("$.data").isEmpty());
    }
